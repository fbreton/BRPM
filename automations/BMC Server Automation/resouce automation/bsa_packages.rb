...
def execute(script_params, parent_id, offset, max_records)
  ...
  if parent_id.blank?
    ...
    data = []
    groups = BsaUtilities.get_child_objects_from_parent_group(BSA_BASE_URL, BSA_USERNAME, BSA_PASSWORD, BSA_ROLE, parent_id.split("|")[2], parent_id.split("|")[1], "DEPOT_GROUP")
    if groups
      groups.each do |group|
        data << { :title => group["name"], :key => "#{group["groupId"]}|#{group["objectId"]}|#{group["modelType"]}", :isFolder => true, :hasChild => true, :hideCheckbox => true}
      end
    end
    groups = BsaUtilities.get_child_objects_from_parent_group(BSA_BASE_URL, BSA_USERNAME, BSA_PASSWORD, BSA_ROLE, parent_id.split("|")[2], parent_id.split("|")[1], "SMART_DEPOT_GROUP")
    if groups
      groups.each do |group|
        data << { :title => group["name"], :key => "#{group["groupId"]}|#{group["objectId"]}|#{group["modelType"]}", :isFolder => true, :hasChild => true, :hideCheckbox => true}
      end
    end
    objects = BsaUtilities.get_child_objects_from_parent_group(BSA_BASE_URL, BSA_USERNAME, BSA_PASSWORD, BSA_ROLE, parent_id.split("|")[2], parent_id.split("|")[1], "BLPACKAGE")
    if objects
