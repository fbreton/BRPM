...
def execute(script_params, parent_id, offset, max_records)
  if parent_id.blank?
    # root folder
	data = [{ :title => "/BRPM/Deploy", :key => "NormalizedPath", :isFolder => false }]
    group = BsaUtilities.get_root_group(BSA_BASE_URL, BSA_USERNAME, BSA_PASSWORD, BSA_ROLE, "JOB_GROUP")
    ...
    groups = BsaUtilities.get_child_objects_from_parent_group(BSA_BASE_URL, BSA_USERNAME, BSA_PASSWORD, BSA_ROLE, "JOB_GROUP", parent_id.split("|")[1], "JOB_GROUP")
